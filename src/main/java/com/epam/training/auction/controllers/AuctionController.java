...
public final class AuctionController {
    ...
    public ModelAndView getAuctions() {
        ...
        activeAuctions.add(AuctionTransferObject.getBuilder("Cool old shoes", null).setId(1L)
                .setDescription("Stink a little").build());
        activeAuctions
                .add(AuctionTransferObject.getBuilder("Nothing", null).setId(2L).setDescription("Some air").build());
        activeAuctions
                .add(AuctionTransferObject.getBuilder("Gold bar", null).setId(3L).setDescription("Only 5$").build());
    ...
    public String addAuction(@RequestParam String title, @RequestParam String description,
            @AuthenticationPrincipal User currentUser) {
        ModelAndView model = new ModelAndView();
        UserTransferObject userTransferObject = new UserTransferObject(currentUser.getId(), currentUser.getUsername(),
                currentUser.getPassword());
        AuctionTransferObject auction = AuctionTransferObject.getBuilder(title, userTransferObject)
    ...
    public ModelAndView getAuction(@PathVariable Long auctionId) {
        ...
        AuctionTransferObject auction = AuctionTransferObject.getBuilder("Example auction", null)
                .setDescription("Hello. Is it me you're looking for?").setId(auctionId).build();
    ...
    public String doBid(@RequestParam Long auctionId, @RequestParam Long amount,
            @AuthenticationPrincipal User currentUser, RedirectAttributes redirectAttrs) {
        // this.biddingService.bid(auctionId, amount, 0);
        redirectAttrs.addFlashAttribute("auctionId", auctionId).addFlashAttribute("message",
